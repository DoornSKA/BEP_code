...
class plot_data():
    ...
    def plot_data(data, response):
        time_from_samples = [i/8000 for i in range(len(data))]
        # plt.plot(data)
        plt.plot(time_from_samples, response)
        # plt.legend(["Original", "Filtered"])
        plt.xlabel('time [s]')
        plt.ylabel('Voltage in bits compared to reference voltage')
        plt.legend(["Filtered"])
        plt.show()
